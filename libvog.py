...
def repo_create(path):
    repo = VogRepo(path, True)  # only create an object doesn't create a dir
...
#find the repo directory root since we may be working in a subdirectory
def repo_find(path=".", required=True):
    path = os.path.realpath(path)

    #if .git folder is found that means we are in the root
    if os.path.isdir(os.path.join(path, ".git")):
        return VogRepo(path) # return the repo object by creating one
...
class VogObject(object):
    ...
    def serialize(self, repo):
        """This function MUST be implemented by subclasses.It must read the object's contents from self.data, a byte string, and do
...
# hash determines the path and filename of object. We need that to read the object(file)
def object_read(repo, sha):
    ...
    with open(path,"rb") as f:
        ...
        match fmt:
            case b'commit' : c=VogCommit
            case b'tree'   : c=VogTree
            case b'tag'    : c=VogTag
            case b'blob'   : c=VogBlob
            case _:
...
def object_write(obj, repo=None):
    # Serialize object data. Convert to hex
    data = obj.serialize()
    # Add header
...
def cmd_cat_file(args):
    repo = repo_find() #find the repo object
...
def object_hash(fd, fmt, repo=None):
    ...
    match fmt:
        case b'commit' : obj=VogCommit(data)
        case b'tree'   : obj=VogTree(data)
        case b'tag'    : obj=VogTag(data)
        case b'blob'   : obj=VogBlob(data)
...
def tree_parse_one(raw, start=0):
    ...
    return y+21, VogTreeLeaf(mode, path.decode("utf8"), sha)
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        entries.append(VogIndexEntry(ctime=(ctime_s, ctime_ns),
    ...
    return VogIndex(version=version, entries=entries)
...
def gitignore_read(repo):
    ret = VogIgnore(absolute=list(), scoped=dict())
...
def add(repo, paths, delete=True, skip_missing=False):
  ...
  for path in paths:
    ...
    for (abspath, relpath) in clean_paths:
      ...
      entry = VogIndexEntry(ctime=(ctime_s, ctime_ns), mtime=(mtime_s, mtime_ns), dev=stat.st_dev, ino=stat.st_ino,
...
def tree_from_index(repo, index):
    ...
    for path in sorted_paths:
        ...
        for entry in contents[path]:
            # An entry can be a normal VogIndexEntry read from the
            # index, or a tree we've created.
            if isinstance(entry, VogIndexEntry): # Regular entry (a file)
                ...
                leaf = VogTreeLeaf(mode = leaf_mode, path=os.path.basename(entry.name), sha=entry.sha)
            else: # Tree.  We've stored it as a pair: (basename, SHA)
                leaf = VogTreeLeaf(mode = b"040000", path=entry[0], sha=entry[1])
...
def commit_create(repo, tree, parent, author, timestamp, message):
    commit = VogCommit() # Create the new commit object.
...
def cmd_commit(args):
    ...
    commit = commit_create(repo,
                           ...
                           vogconfig_user_get(vogconfig_read()),
                           datetime.now(),
                           args.message)
